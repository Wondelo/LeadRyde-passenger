...
public class RatingActivity extends AppCompatActivity {
    ...
    private String skipRideRating;
    private String skipSafetyRating;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        rideRatingArea.setVisibility(View.VISIBLE);
...
//
    ...
    public void submitRating(String skipRideRating, String skipSafetyRating) {
        HashMap<String, String> parameters = new HashMap<String, String>();
        ...
        parameters.put("UserType", Utils.app_type);
        parameters.put("skipRideRating", skipRideRating);
        parameters.put("skipSafetyRating", skipSafetyRating);
    ...
    public void buildTipCollectMessage(String message, String positiveBtn, String negativeBtn, String skipRideRating, String skipSafetyRating) {
        ...
        skipTxtArea.setOnClickListener(view -> {
            ...
            submitRating(skipRideRating,skipSafetyRating);
            isCollectTip = false;
        });

        giveTipTxtArea.setOnClickListener(view -> {
            giveTipAlertDialog.dismiss();
            showTipBox(skipRideRating, skipSafetyRating);
    ...
    public void showTipBox(String skipRideRating, String skipSafetyRating) {
        androidx.appcompat.app.AlertDialog.Builder builder = new androidx.appcompat.app.AlertDialog.Builder(getActContext());
        LayoutInflater inflater = (LayoutInflater) getActContext().getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        View dialogView = inflater.inflate(R.layout.desgin_add_tip_layout, null);
        builder.setView(dialogView);
    ...
    public class setOnClickList implements View.OnClickListener {
        ...
        public void onClick(View view) {
            ...
            Utils.hideKeyboard(getActContext());
            if (i == submitBtnId) {
                if (ratingBar.getRating() < 0.5 && ratingBar != null && ratingBar.getVisibility() == View.VISIBLE && generalFunc.getJsonValue("ENABLE_RATING", userProfileJson).equalsIgnoreCase("Yes")) {
                ...
                if(generalFunc.getJsonValue("ENABLE_RATING", userProfileJson).equalsIgnoreCase("Yes") && ratingBar.getRating() < 0.5 && ratingBar != null && ratingBar.getVisibility() == View.VISIBLE){
                  skipRideRating = "No";
                }else{
                  skipRideRating = "Yes";
                }

              if(generalFunc.getJsonValue("ENABLE_SAFETY_RATING", userProfileJson).equalsIgnoreCase("Yes") && safetyRatingArea != null && safetyRatingArea.getVisibility() == View.VISIBLE && safetyRatingBar.getRating() < 0.5){
                skipSafetyRating = "No";
              }else{
                skipSafetyRating = "Yes";
              }

                if (isCollectTip) {
                    buildTipCollectMessage(generalFunc.retrieveLangLBl("", "LBL_TIP_TXT"), generalFunc.retrieveLangLBl("No,Thanks", "LBL_NO_THANKS"),
                            generalFunc.retrieveLangLBl("Give Tip", "LBL_GIVE_TIP_TXT"), skipRideRating, skipSafetyRating);
                    return;
                } else {
                    btn_type2.setEnabled(false);
                    submitRating(skipRideRating, skipSafetyRating);
