 ...
 */
public class LoadAvailableCab implements UpdateFrequentTask.OnTaskRunCalled, MapDelegate {
    ...
    float DRIVER_ARRIVED_MIN_TIME_PER_MINUTE = 3;
...
//        }
            ...
            if (responseString != null && !responseString.equals("")) {
                ...
                if (availCabArr != null) {
                    for (int i = 0; i < availCabArr.length(); i++) {
                        ...
                        if(generalFunc.getJsonValue("ENABLE_AVAILABLE_CABS_ETA", userProfileJson).equalsIgnoreCase("Yes")){
                            calculateETA();
                           /* calculateETA2();*/
                        }
                    }
    ...
    public void calculateETA() {
        ...
        for (int i = 0; i < listOfDrivers.size(); i++) {
           ...
           /* lowestTime = ((int) (lowestKM * DRIVER_ARRIVED_MIN_TIME_PER_MINUTE));
            ...
            if (mainAct.cabSelectionFrag != null) {
                if (mainAct.cabSelectionFrag.cabTypeList != null) {
                    Logger.d("CALCULATE_ETA", ":SIZE::" +mainAct.cabSelectionFrag.cabTypeList.size());
                    for (int j = 0; j < mainAct.cabSelectionFrag.cabTypeList.size(); j++) {
                        if (listOfDrivers.get(i).get("vCarType").contains(mainAct.cabSelectionFrag.cabTypeList.get(j).get("iVehicleTypeId"))) {
                            HashMap hashMap = mainAct.cabSelectionFrag.cabTypeList.get(j);
                            hashMap.put("timeTxt", getTimeTxt(GeneralFunctions.parseIntegerValue(0, String.valueOf(lowestTime))));
                            mainAct.cabSelectionFrag.cabTypeList.set(j, hashMap);
                            Logger.d("CALCULATE_ETA", ":sssssssss::" + mainAct.cabSelectionFrag.cabTypeList.get(j));
                        }
        ...
        if (mainAct.cabSelectionFrag != null) {
            if (mainAct.cabSelectionFrag.adapter != null) {
                mainAct.cabSelectionFrag.adapter.notifyDataSetChanged();
            }
        }


    }
    public void calculateETA2() {
        for (int i = 0; i < listOfDrivers.size(); i++) {
            HashMap<String, String> driverData = listOfDrivers.get(i);
            String originLoc = driverData.get("Latitude") + "," + driverData.get("Longitude");
            String destLoc = mainAct.pickUpLocation.getLatitude() + "," + mainAct.pickUpLocation.getLongitude();
            ...
            hashMap.put("d_latitude", mainAct.pickUpLocation.getLatitude() + "");
            hashMap.put("d_longitude", mainAct.pickUpLocation.getLongitude() + "");


            hashMap.put("s_latitude", driverData.get("Latitude"));
            hashMap.put("s_longitude",  driverData.get("Longitude"));
    ...
    public void filterDrivers(boolean isCheckAgain) {
        ...
        for (int i = 0; i < listOfDrivers.size(); i++) {
            ...
            Logger.d("CALCULATE_ETA", ""  + driverName);
...
//            calculateMinimumPickUpTime(driverData, selectedCabTypeId);
    ...
    public void directionResult(HashMap<String, String> directionlist) {
        ...
        if (responseString != null && !responseString.equalsIgnoreCase("") && directionlist.get("distance") == null) {
            ...
            if (obj_routes != null && obj_routes.length() > 0) {
                ...
                String time = "" + (generalFunc.getJsonValue("value", generalFunc.getJsonValue("duration", obj_legs.toString()).toString()));
                for (int i = 0; i < listOfDrivers.size(); i++) {
                    if (mainAct.cabSelectionFrag != null) {
                        if (mainAct.cabSelectionFrag.cabTypeList != null) {
                            Logger.d("CALCULATE_ETA", ":SIZE::" +mainAct.cabSelectionFrag.cabTypeList.size());
                            for (int j = 0; j < mainAct.cabSelectionFrag.cabTypeList.size(); j++) {
                                if (listOfDrivers.get(i).get("vCarType").contains(mainAct.cabSelectionFrag.cabTypeList.get(j).get("iVehicleTypeId"))) {
                                    HashMap hashMap = mainAct.cabSelectionFrag.cabTypeList.get(j);
                                    hashMap.put("timeTxt",getTimeTxt(GeneralFunctions.parseIntegerValue(0,time)));
                                    mainAct.cabSelectionFrag.cabTypeList.set(j, hashMap);
                                }
                ...
                if (mainAct.cabSelectionFrag != null) {
                    if (mainAct.cabSelectionFrag.cabTypeList != null) {
                        ...
                        for (int j = 0; j < mainAct.cabSelectionFrag.cabTypeList.size(); j++) {
                            if (listOfDrivers.get(i).get("vCarType").contains(mainAct.cabSelectionFrag.cabTypeList.get(j).get("iVehicleTypeId"))) {
                                HashMap hashMap = mainAct.cabSelectionFrag.cabTypeList.get(j);
                                hashMap.put("timeTxt",duration+" "+durationTxt);
                                mainAct.cabSelectionFrag.cabTypeList.set(j, hashMap);
                                Logger.d("CALCULATE_ETA", ":sssssssss::" + mainAct.cabSelectionFrag.cabTypeList.get(j));
                            }
