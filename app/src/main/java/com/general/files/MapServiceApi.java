...
public class MapServiceApi {
    ...
    public static void getAutoCompleteService(Context context, HashMap<String, String> mapdictionary, MapDelegate mapDelegate) {
        ...
        ExecuteWebServerUrl exeWebServer = new ExecuteWebServerUrl(context, url, true,mapdictionary.get("latitude") + "," + mapdictionary.get("longitude"));
...
//                                hashMap.put("routes", generalFunc.getJsonArray("routes", responseString) + "");
    ...
    public static void getGeoCodeService(Context context, HashMap<String, String> mapdictionary, MapDelegate mapDelegate) {
       ...
       String mapStrategy = generalFunc.retrieveValue("MAPS_API_REPLACEMENT_STRATEGY");
        ...
        ExecuteWebServerUrl exeWebServer = new ExecuteWebServerUrl(context, url_str, true,mapdictionary.get("latitude") + "," + mapdictionary.get("longitude"));
                ...
                if (mapStrategy.equalsIgnoreCase("Advance")) {
                    ...
                    if (status.equals("OK")) {
                        ...
                        if (arr.length() > 0) {
                            ...
                            /*JSONArray addressArray = generalFunc.getJsonArray("address_components", obj);
                            boolean first_input = true;
                            for (int j = 0; j < addressArray.length(); j++) {
                                JSONObject addObject = generalFunc.getJsonObject(addressArray, j);
                                String long_name = generalFunc.getJsonValueStr("long_name", addObject);
                                if (!long_name.contains("Unnamed") && !long_name.contains("null")) {

                                    *//*if (j == 0 && long_name.matches("^[0-9]+$")) {
                                        continue;
                                    }*//*
                                    if (first_input) {
                                        address_loc = long_name;
                                        first_input = false;
                                    } else {
                                        if (j == 0 || j == 1) address_loc = address_loc + "," + long_name;
                                    }
                                }
                            }*/

                            /** Start : Below code commented for addon that address need small */
                            ...
                            for (int i = 0; i < addressArr.length; i++) {
                                if (!addressArr[i].contains("Unnamed") && !addressArr[i].contains("null")) {
                                    ...
                                    if (first_input) {
                                        ...
                                        /*if (1 == 0 || i == 1) */address_loc = address_loc + "," + addressArr[i];
                                    }

                                }
                            }
                            /** End : Below code commented for addon that address need small */
